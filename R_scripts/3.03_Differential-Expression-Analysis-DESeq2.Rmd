...
smoc_res_lfc_cutoff_all <- data.frame(smoc_res_lfc_cutoff) %>% 
              rownames_to_column(var = "ensgene") %>%
              left_join(x = smoc_res_lfc_cutoff, 
                        y = grcm38[, c("ensgene", "symbol", "description")], 
                        by = "ensgene") 


smoc_res_lfc_cutoff_all_sig <- subset(smoc_res_lfc_cutoff_all, padj < 0.05)
```
